...
class HaloDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        dens_patch = (dens_patch - dens_patch.mean()) / (dens_patch.std() + 1e-6)
        pres_patch = (pres_patch - pres_patch.mean()) / (pres_patch.std() + 1e-6)
        
        # stack channels 
        x_all = np.stack([dens_patch,  pres_patch], axis=0)
...
def evaluate_model(model, te_loader, test_ds,
    ...
    plt.tight_layout()
    plt.savefig("same_real_cnn_density_pres.png")
    plt.show()
