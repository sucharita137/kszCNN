...
INPUT_TYPES = {
    ...
    "overdensity+nsub": "Galaxy Overdensity + N_subs"
}
...
def make_title(set_name, input_type, exp_type):
    ...
    if input_type == "position":
        label = r"$n_g$"
    elif input_type == "overdensity+nsub":
        label = r"$\delta_g + nsub$"
    else:
        label = input_type
...
def load_feature_file(hfile, feature_type="nsubs"):
...
def evaluate_model(model, te_loader, test_ds,
    ...
    plt.tight_layout()
    plt.savefig("same_real_cnn_overdensity_N_subs.png")
    plt.show()
