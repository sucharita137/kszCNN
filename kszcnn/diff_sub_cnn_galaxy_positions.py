...
set_name = "SIMBA_1P_0"
input_type = "position"
exp_type = "diff_subgrid"

text_halo_filename, params = load_cosmology(set_name)

TEST_HALO_FILE = os.path.join(DATA_DIR, text_halo_filename)
...
class HaloDatasetGalPos(Dataset):

    def __init__(self, halo_pos, halo_vel, halo_mass,
                 sub_pos, sub_mstar,
                 patch=PATCH, boxsize=BOXSIZE,
                 mass_cut=MASS_CUT, mstar_cut=1e8,rng=None):
        if rng is None:
            rng = np.random.RandomState(SEED)
        mask = halo_mass > mass_cut
        self.halo_pos = halo_pos[mask]
        self.vz = halo_vel[mask][:,2].astype(np.float32)

        gal_mask = sub_mstar > mstar_cut
        self.sub_pos = sub_pos[gal_mask]

        self.patch = patch
        self.boxsize = boxsize

        print(f"Selected halos: {len(self.halo_pos)}")
        print(f"Selected galaxies: {len(self.sub_pos)}")

    def __len__(self):
        return len(self.halo_pos)

    def __getitem__(self, i):

        center = self.halo_pos[i]

        # relative positions 
        rel = self.sub_pos - center
        rel = (rel + self.boxsize/2) % self.boxsize - self.boxsize/2

        # voxelization
        grid = np.zeros((self.patch, self.patch, self.patch), dtype=np.float32)

        cell = self.boxsize / self.patch

        idx = ((rel + self.boxsize/2) / cell).astype(int)

        mask = (
            (idx[:,0] >= 0) & (idx[:,0] < self.patch) &
            (idx[:,1] >= 0) & (idx[:,1] < self.patch) &
            (idx[:,2] >= 0) & (idx[:,2] < self.patch)
        )
        ...
        grid = np.log1p(grid)
        x = torch.tensor(grid[None], dtype=torch.float32)
...
def train_model(model, tr_loader, val_loader, y_mean, y_std):
    ...
    loss_fn = nn.HuberLoss(delta=50)  # nn.SmoothL1Loss()
...
def evaluate_model(model, te_loader, test_ds,
    ...
    print(f"corr.coef r = {corr:.2f}")
    print(f"m = {m:.2f}")
    print(f"b = {b:.2f}")
    print(f"metric_val = {metric_val:.2f}")
    ...
    plt.tight_layout()
    plt.savefig("diff_sub_cnn_galaxy_pos.png")
    plt.show()
