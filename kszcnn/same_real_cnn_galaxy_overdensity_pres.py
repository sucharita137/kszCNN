...
INPUT_TYPES = {
    ...
    "overdensity+pres": "Galaxy Overdensity + Pressure"
}
...
def make_title(set_name, input_type, exp_type):
    ...
    if input_type == "position":
        label = r"$n_g$"
    elif input_type == "overdensity+pres":
        label = r"$\delta_g + Pressure$"
    else:
        label = input_type
...
GRID_P_FILE = os.path.join(DATA_DIR, "Grids_P_IllustrisTNG_1P_128_z=0.0.npy")
REAL_IDX=0
# In[4]:
...
class HaloDataset(Dataset):
    ...
    def __init__(self, density_grid, pos, vel,mass,pressure_grid,
    ...
    def __getitem__(self, i):
        ...
        pres_patch = (pres_patch - pres_patch.mean()) / (pres_patch.std() + 1e-6)
        x_all = np.stack([patch,  pres_patch], axis=0)
...
def evaluate_model(model, te_loader, test_ds,
    ...
    plt.tight_layout()
    plt.savefig("same_real_cnn_overdensity_pres.png")
    plt.show()
